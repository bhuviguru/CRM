...
def calculate_shap_values(input_data, prediction_proba):
    ...
    # Calculate normalized impact (simplified)
    # Higher tickets = more churn risk (positive impact on churn)
    # Higher usage = less churn risk (negative impact on churn)
    impacts = {
        'support_tickets': features['support_tickets'] * 0.15,
        'email_response_time': features['email_response_time'] * 0.05,
        'usage_frequency': -features['usage_frequency'] * 0.02,
    ...
    # Normalize to sum to prediction probability
    total_impact = sum(abs(v) for v in impacts.values())
    if total_impact > 0:
        normalized_impacts = {k: (v / total_impact) * prediction_proba for k, v in impacts.items()}
    else:
...
def generate_reasoning(top_factors, risk_level):
    """Generate human-readable explanation"""
    if risk_level == "Critical" or risk_level == "High":
        primary = top_factors[0]
        secondary = top_factors[1] if len(top_factors) > 1 else None
        
        reasoning = f"High churn risk detected. "
        
        if primary['direction'] == 'positive':
            reasoning += f"Primary concern: {primary['feature'].replace('_', ' ')} is elevated ({primary['feature_value']}). "
        else:
            reasoning += f"Primary concern: {primary['feature'].replace('_', ' ')} is low ({primary['feature_value']}). "
        
        if secondary:
            reasoning += f"Additionally, {secondary['feature'].replace('_', ' ')} shows {secondary['direction']} trend."
        
        return reasoning
    
    elif risk_level == "Medium":
        return "Moderate churn risk. Customer engagement shows mixed signals. Proactive outreach recommended."
    
    else:
        return "Low churn risk. Customer shows healthy engagement patterns and satisfaction levels."

def generate_recommendations(top_factors, risk_level):
    """Generate actionable recommendations"""
    recommendations = []
    
    if risk_level in ["Critical", "High"]:
        # Check top negative factors
        for factor in top_factors[:2]:
            if factor['feature'] == 'support_tickets' and factor['direction'] == 'positive':
                recommendations.append("Schedule technical review call to address support issues")
            elif factor['feature'] == 'email_response_time' and factor['direction'] == 'positive':
                recommendations.append("Improve response time - assign dedicated CSM")
            elif factor['feature'] == 'usage_frequency' and factor['direction'] == 'negative':
                recommendations.append("Offer product training session to boost engagement")
            elif factor['feature'] == 'contract_value' and factor['direction'] == 'negative':
                recommendations.append("Explore upsell opportunities to increase account value")
        
        recommendations.append("Schedule executive business review within 7 days")
    
    elif risk_level == "Medium":
        recommendations.append("Monitor customer health score weekly")
        recommendations.append("Send product update newsletter")
...
def predict_churn(data: CustomerData):
    ...
    # Calculate explainability
    top_factors = calculate_shap_values(data.dict(), probability)
    reasoning = generate_reasoning(top_factors, risk_level)
    recommendations = generate_recommendations(top_factors, risk_level)
    
    # Calculate confidence (simplified - in production use model calibration)
    confidence = 0.85 + (abs(probability - 0.5) * 0.3)  # Higher confidence at extremes
    confidence = min(confidence, 0.99)
